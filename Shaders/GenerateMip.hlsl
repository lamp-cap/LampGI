...
float NumMipLevels  : register(b1);	// Number of OutMips to write: [1, 4]
float rcpWidth      : register(b2);
float rcpHeight     : register(b3);	// 1.0 / OutMip1.Dimensions
...
void CS( uint GI : SV_GroupIndex, uint3 DTid : SV_DispatchThreadID )
{
    ...
    float4 Src1 = SrcMip.SampleLevel(gsamLinearClamp, UV, SrcMipLevel);
    ...
    float4 Src1 = 0.5 * (SrcMip.SampleLevel(gsamLinearClamp, UV1, SrcMipLevel) +
        SrcMip.SampleLevel(gsamLinearClamp, UV1 + Off, SrcMipLevel));
    ...
    float4 Src1 = SrcMip.SampleLevel(gsamLinearClamp, UV1, SrcMipLevel);
    Src1 += SrcMip.SampleLevel(gsamLinearClamp, UV1 + float2(O.x, 0.0), SrcMipLevel);
    Src1 += SrcMip.SampleLevel(gsamLinearClamp, UV1 + float2(0.0, O.y), SrcMipLevel);
    Src1 += SrcMip.SampleLevel(gsamLinearClamp, UV1 + float2(O.x, O.y), SrcMipLevel);
    Src1 *= 0.25;
#endif
